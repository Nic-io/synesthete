...
void *dmadaccall(const struct device *dev, void *user_data, uint32_t channel,
		 int status)
{
	if (status != 0) {
...
int dacout_init(uint16_t *buffer, uint16_t size)
{
	...
	static const struct dac_channel_cfg dac_ch_cfg = {
		.channel_id = DAC_CHANNEL_ID, .resolution = DAC_RESOLUTION
	};
...
#warning "change the way to get the address of DHR"
	...
	struct dma_config dacdma_cfg = { .dma_slot = 7,
					 .channel_direction =
