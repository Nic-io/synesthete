...
int gui_init(void)
{
	...
	if (display_dev == NULL) {
		LOG_ERR("Device %s not found. Aborting.", DISPLAY_DEV_NAME);
		return 1;
	}
	...
	for (size_t i = 0; i < size; i++) {
		...
		if (*(newflag + i) == 1) {
			...
			*(newflag + i) = 0;
			...
			buf_desc.buf_size =
				buf_desc.height * buf_desc.width * 3;
			buf_desc.pitch = buf_desc.width;

			fill_buffer(&color);
			display_write(display_dev, buf_desc.width * i, 0,
				      &buf_desc, buf);
			...
			if (*(table + i) > (capabilities.y_resolution / 2)) {
				color = 0x3F9F16;
				buf_desc.height = *(table + i) -
						  capabilities.y_resolution / 2;
				buf_desc.buf_size =
					(buf_desc.height * buf_desc.width) * 3;
				fill_buffer(&color);
				display_write(display_dev, buf_desc.width * i,
					      capabilities.y_resolution / 2,
					      &buf_desc, buf);
			}
...
void gui_draw_point(int x, int y)
{
	...
	if (x > 0 && x < capabilities.x_resolution && y > 0 &&
	    y < capabilities.y_resolution) {
